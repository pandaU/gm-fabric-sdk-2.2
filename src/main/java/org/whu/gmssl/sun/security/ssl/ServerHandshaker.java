...
import org.whu.gmssl.sun.security.ssl.CipherSuite.KeyExchange;
import org.whu.gmssl.sun.security.ssl.HandshakeMessage.*;
import org.whu.gmssl.sun.security.ssl.SignatureAndHashAlgorithm.HashAlgorithm;
import org.whu.gmssl.sun.security.ssl.SignatureAndHashAlgorithm.SignatureAlgorithm;
import org.whu.gmssl.sun.security.util.KeyUtil;
...
import static org.whu.gmssl.sun.security.ssl.CipherSuite.KeyExchange.*;
...
final class ServerHandshaker extends Handshaker {
    ...
    private boolean singleCert = false;
    ...
    void processMessage(byte type, int message_len)
        ...
        switch (type) {
            ...
            case HandshakeMessage.ht_client_key_exchange:
                ...
                case K_SM2_SM2:
                    //TODO SM2KeyExchange
//                    preMasterSecret = this.clientKeyExchange(new SM2ClientKeyExchange(this.input));
                    throw new IllegalStateException("K_SM2_SM2暂不支持");
                case K_ECC:
                    ECCClientKeyExchange eccClientKeyExchange = new ECCClientKeyExchange(this.protocolVersion, this.clientRequestedVersion, this.sslContext.getSecureRandom(), this.input, message_len, this.encPrivateKey);
    ...
    private void clientHello(ClientHello mesg) throws IOException {
        ...
        if (keyExchange == K_KRB5 || keyExchange == K_KRB5_EXPORT) {
            ...
            if (this.protocolVersion.isIsgm() && this.protocolVersion.minor == 2) {
                ...
                if (encCerts == null || encCerts.length == 0){
                    x509Certificates = new X509Certificate[]{certs[0]};
                }else{
                    x509Certificates = new X509Certificate[this.certs.length + 1];
                    x509Certificates[0] = certs[0];
                    x509Certificates[1] = encCerts[0];

                    System.arraycopy(this.certs, 1, x509Certificates, 2, this.certs.length - 1);
                }
                m2 = new CertificateMsg(x509Certificates);
                session.setLocalCertificates(x509Certificates);
            } else {
                m2 = new CertificateMsg(certs);
...
//            String var27 = System.getProperty("ecc_server_key_exchange");
            ...
            if (var27 != null) {
                try {
                    m3 = new ECCServerKeyExchange(this.privateKey, this.certs[0].getPublicKey(), this.clnt_random, this.svr_random, this.encCerts[0], this.sslContext.getSecureRandom());
    ...
    private void chooseCipherSuite(ClientHello mesg) throws IOException {
        ...
        Collection<CipherSuite> cipherSuites = prefered.collection();

        //TODO 测试让国密算法在最前面
        List<CipherSuite> gm = new ArrayList<>();
        List<CipherSuite> normal = new ArrayList<>();
        List<CipherSuite> finalSuites = new ArrayList<>();

        for (CipherSuite cipherSuite : cipherSuites){
            if (cipherSuite.name.toUpperCase().contains("SM4")){
                gm.add(cipherSuite);
            }else {
                normal.add(cipherSuite);
            }
        }

        finalSuites.addAll(gm);
        finalSuites.addAll(normal);

        for (CipherSuite suite : finalSuites) {
            if (!isNegotiable(proposed, suite)) {
                continue;
            }
    ...
    boolean trySetCipherSuite(CipherSuite suite) {
        ...
        if (protocolVersion.v >= suite.obsoleted && !suite.name.contains("SM4")) {
            return false;
        }
...
//            break;
    ...
    //TODO sm2秘钥协商暂不支持 modify by ringo
...
//        this.sm2 = new SM2Crypt(this.encCerts[0].getPublicKey(), this.encPrivateKey, this.sslContext.getSecureRandom(), true);
    ...
    private boolean setupPrivateKeyAndChain(String algorithm) {
        ...
        if (sepIndex == -1){
            ...
            if (!protocolVersion.isIsgm() && keyAlgorithm.equals("EC")) {
            ...
            String alias1 = alias.substring(0, sepIndex);
            String alias2 = alias.substring(sepIndex + 1);

            PrivateKey tempPrivateKey1 = km.getPrivateKey(alias1);

            if (tempPrivateKey1 == null) {
                return false;
            }

            PrivateKey tempPrivateKey2 = km.getPrivateKey(alias2);

            if (tempPrivateKey2 == null) {
                return false;
            }

            X509Certificate[] tempCerts1 = km.getCertificateChain(alias1);
            if ((tempCerts1 == null) || (tempCerts1.length == 0)) {
                return false;
            }

            X509Certificate[] tempCerts2 = km.getCertificateChain(alias2);
            if ((tempCerts2 == null) || (tempCerts2.length == 0)) {
                return false;
            }
            if (encryptCert(tempCerts1[0]) && signCert(tempCerts2[0])) {
                this.encPrivateKey = tempPrivateKey1;
                this.encCerts = tempCerts1;
                this.privateKey = tempPrivateKey2;
                this.certs = tempCerts2;
            } else if (encryptCert(tempCerts2[0]) && signCert(tempCerts1[0])) {
                this.encPrivateKey = tempPrivateKey2;
                this.encCerts = tempCerts2;
                this.privateKey = tempPrivateKey1;
                this.certs = tempCerts1;
            } else {
...
//    }
    ...
    static class SuiteComparator implements Comparator<CipherSuite> {
        @Override
        public int compare(CipherSuite o1, CipherSuite o2) {
            if (o1.name.contains("SM4") && !o2.name.contains("SM4")) {
                return -1;
            } else {
                return !o1.name.contains("SM4") && o2.name.contains("SM4") ? 1 : o1.name.compareTo(o2.name);
            }
        }
    }
}
