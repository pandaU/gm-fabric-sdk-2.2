...
import org.whu.gmssl.sun.security.ssl.CipherSuite.PRF;
...
import static org.whu.gmssl.sun.security.ssl.CipherSuite.PRF.P_NONE;
...
static final class CertificateVerify extends HandshakeMessage {
    ...
    boolean verify(ProtocolVersion protocolVersion,
        ...
        //TODO modify by ringo
        if (protocolVersion.isIsgm()) {
            sig = JsseJce.getSignature("SM3withSM2");
        }else if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
            sig = JsseJce.getSignature(
                        preferableSignatureAlgorithm.getAlgorithmName());
        } else {
    ...
    private static void updateSignature(Signature sig,
        ...
        if (algorithm.equals("RSA")) {
            ...
            if (protocolVersion.v < ProtocolVersion.TLS12.v) { // TLS1.1-
                MessageDigest digestClone = handshakeHash.getSHAClone();

                //TODO modify by ringo
                if (protocolVersion.isIsgm()) {
                    digestClone = handshakeHash.getSM3Clone();
                }else if (protocolVersion.v < ProtocolVersion.TLS10.v) { // SSLv3
                    updateDigest(digestClone, SHA_pad1, SHA_pad2, masterKey);
                }

                sig.update(digestClone.digest());
            } else {  // TLS1.2+
                sig.update(handshakeHash.getAllHandshakeMessages());
            }
        }
...
static final class Finished extends HandshakeMessage {
                ...
                if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                    ...
                    if (protocolVersion.isIsgm()) {
                        if (protocolVersion.minor == 0) {
                            MessageDigest shaClone = handshakeHash.getSHAClone();
                            MessageDigest sm3Clone = handshakeHash.getSM3Clone();
                            seed = new byte[52];
                            shaClone.digest(seed, 0, 20);
                            sm3Clone.digest(seed, 20, 32);
                        } else {
                            if (this.protocolVersion.minor != 2) {
                                throw new RuntimeException("unsupported ssl version");
                            }

                            MessageDigest sm3Clone = handshakeHash.getSM3Clone();
                            seed = new byte[32];
                            sm3Clone.digest(seed, 0, 32);
                        }
    ...
    static final class SM2_ServerKeyExchange extends ServerKeyExchange {
