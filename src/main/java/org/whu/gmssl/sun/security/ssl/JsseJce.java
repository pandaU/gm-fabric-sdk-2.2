...
import org.whu.gmssl.sun.security.util.ECUtil;
...
final class JsseJce {
    ...
    static Cipher getCipher(String transformation)
        ...
        try {
            if (GMJsse.cryptoProvider == null) {
                return Cipher.getInstance(transformation);
            } else {
                return Cipher.getInstance(transformation, GMJsse.cryptoProvider);
            }
    ...
    static Signature getSignature(String algorithm)
        ...
        if (GMJsse.cryptoProvider == null) {
            ...
            if (algorithm == SIGNATURE_SSLRSA) {
                ...
                // the GMJsse implementation does the actual crypto using
                // a NONEwithRSA signature obtained from the cryptoProvider.
                if (GMJsse.cryptoProvider.getService("Signature", algorithm) == null) {
                    ...
                    try {
                        return Signature.getInstance(algorithm, "GMJsse");
                    } catch (NoSuchProviderException e) {
                        throw new NoSuchAlgorithmException(e);
                    }
                }
            }
            return Signature.getInstance(algorithm, GMJsse.cryptoProvider);
        }
    }
    ...
    static KeyGenerator getKeyGenerator(String algorithm)
        ...
        if (GMJsse.cryptoProvider == null) {
            ...
            return KeyGenerator.getInstance(algorithm, GMJsse.cryptoProvider);
        }
    }
    ...
    static KeyPairGenerator getKeyPairGenerator(String algorithm)
        ...
        if (GMJsse.cryptoProvider == null) {
            ...
            return KeyPairGenerator.getInstance(algorithm, GMJsse.cryptoProvider);
        }
    }
    ...
    static KeyAgreement getKeyAgreement(String algorithm)
        ...
        if (GMJsse.cryptoProvider == null) {
            ...
            return KeyAgreement.getInstance(algorithm, GMJsse.cryptoProvider);
        }
    }
    ...
    static Mac getMac(String algorithm)
        ...
        if (GMJsse.cryptoProvider == null) {
            ...
            return Mac.getInstance(algorithm, GMJsse.cryptoProvider);
        }
    }
    ...
    static KeyFactory getKeyFactory(String algorithm)
        ...
        if (GMJsse.cryptoProvider == null) {
            ...
            return KeyFactory.getInstance(algorithm, GMJsse.cryptoProvider);
        }
    }
    ...
    static SecureRandom getSecureRandom() throws KeyManagementException {
        ...
        try {
            return SecureRandom.getInstance("PKCS11", GMJsse.cryptoProvider);
        } catch (NoSuchAlgorithmException e) {
            // ignore
        }
        for (Provider.Service s : GMJsse.cryptoProvider.getServices()) {
            if (s.getType().equals("SecureRandom")) {
                try {
                    return SecureRandom.getInstance(s.getAlgorithm(), GMJsse.cryptoProvider);
        ...
        throw new KeyManagementException("FIPS mode: no SecureRandom "
            + " implementation found in provider " + GMJsse.cryptoProvider.getName());
    }
    ...
    static MessageDigest getMessageDigest(String algorithm) {
        try {
            if (GMJsse.cryptoProvider == null) {
                ...
                return MessageDigest.getInstance(algorithm, GMJsse.cryptoProvider);
            }
        } catch (NoSuchAlgorithmException e) {
            throw new RuntimeException
    ...
    static ECParameterSpec getECParameterSpec(String namedCurveOid) {
        return ECUtil.getECParameterSpec(GMJsse.cryptoProvider, namedCurveOid);
    }

    static String getNamedCurveOid(ECParameterSpec params) {
        return ECUtil.getCurveName(GMJsse.cryptoProvider, params);
    }
