...
import static org.whu.gmssl.sun.security.ssl.CipherSuite.CipherType.*;
import static org.whu.gmssl.sun.security.ssl.CipherSuite.KeyExchange.*;
import static org.whu.gmssl.sun.security.ssl.CipherSuite.PRF.*;
...
final class CipherSuite implements Comparable<CipherSuite> {
    ...
    final static BulkCipher B_RC4_40  =
        new BulkCipher(JsseJce.CIPHER_RC4,      STREAM_CIPHER,    5, 16,  0, 0, true);
    final static BulkCipher B_RC2_40  =
        new BulkCipher("RC2",           BLOCK_CIPHER,     5, 16,  8, 0, false);
    final static BulkCipher B_DES_40  =
        new BulkCipher(JsseJce.CIPHER_DES,      BLOCK_CIPHER,     5,  8,  8, 0, true);

    // domestic strength ciphers
    final static BulkCipher B_RC4_128 =
        new BulkCipher(JsseJce.CIPHER_RC4,      STREAM_CIPHER,   16,  0,  0, true);
    final static BulkCipher B_DES     =
        new BulkCipher(JsseJce.CIPHER_DES,      BLOCK_CIPHER,     8,  8,  0, true);
    final static BulkCipher B_3DES    =
        new BulkCipher(JsseJce.CIPHER_3DES,     BLOCK_CIPHER,    24,  8,  0, true);
    final static BulkCipher B_IDEA    =
        new BulkCipher("IDEA",          BLOCK_CIPHER,    16,  8,  0, false);
    final static BulkCipher B_AES_128 =
        new BulkCipher(JsseJce.CIPHER_AES,      BLOCK_CIPHER,    16, 16,  0, true);
    final static BulkCipher B_AES_256 =
        new BulkCipher(JsseJce.CIPHER_AES,      BLOCK_CIPHER,    32, 16,  0, true);
    final static BulkCipher B_AES_128_GCM =
        new BulkCipher(JsseJce.CIPHER_AES_GCM,  AEAD_CIPHER,     16, 12,  4, true);
    final static BulkCipher B_AES_256_GCM =
        new BulkCipher(JsseJce.CIPHER_AES_GCM,  AEAD_CIPHER,     32, 12,  4, true);

    //TODO modify by ringo
    static final BulkCipher B_SM4 =
            new BulkCipher("SM4/CBC/NoPadding", BLOCK_CIPHER, 16, 16, 0, true);
    ...
    static {
        ...
        // TODO modify by ringo
        add("ECC_SM4_SM3", 0xe013, --p, KeyExchange.K_ECC, B_SM4, T, max, tls11, PRF.P_NONE);
