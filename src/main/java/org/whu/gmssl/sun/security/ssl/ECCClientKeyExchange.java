...
import org.whu.gmssl.sun.security.ssl.premasterSecret.TlsGMPremasterSecretParameterSpec;
...
final class ECCClientKeyExchange extends HandshakeMessage {
    ...
    ECCClientKeyExchange(ProtocolVersion protocolVersion,
        ...
        if (!publicKey.getAlgorithm().equals("EC") && !publicKey.getAlgorithm().equals("SM2")) {
            throw new SSLKeyException("Public key not of type SM2");
        }
        this.protocolVersion = protocolVersion;

        int major, minor;

        if (maxVersion.v >= ProtocolVersion.TLS11.v) {
        ...
        try {
            String s = "TlsGMPremasterSecret";
            KeyGenerator kg = JsseJce.getKeyGenerator(s);
            kg.init(new TlsGMPremasterSecretParameterSpec(major, minor),
                    generator);
            preMaster = kg.generateKey();

            Cipher cipher = JsseJce.getCipher("SM2");
            cipher.init(Cipher.ENCRYPT_MODE, publicKey, generator);
            encrypted = cipher.doFinal(preMaster.getEncoded());

//            Security.addProvider(new GMJCE());
//            encrypted = SM2Encryption.encrypt_der(SM2Util.toTsgECPublicKey(publicKey), preMaster.getEncoded(), generator);
            int sm3DigestSize = 32;
            int xlen = (SM2P256V1FieldElement.Q.bitLength() + 7) / 8;

            byte[] c1x = new byte[xlen];
            byte[] c1y = new byte[xlen];
            byte[] c2 = new byte[encrypted.length - 1 - xlen * 2 -sm3DigestSize];
            byte[] c3 = new byte[sm3DigestSize];
            System.arraycopy(encrypted, 1, c1x, 0, xlen);
            System.arraycopy(encrypted, xlen + 1, c1y, 0, xlen);
            System.arraycopy(encrypted, xlen * 2 + 1, c2, 0, encrypted.length - 1 - xlen * 2 -sm3DigestSize);
            System.arraycopy(encrypted, encrypted.length - sm3DigestSize, c3, 0, sm3DigestSize);
//            (key instanceof BCECPublicKey) ? ((BCECPublicKey)publicKey).getParams() : ECUtil.generatePublicKeyParameter(publicKey)
//            ECKeyParameters ecKey = (ECKeyParameters)rParam.getParameters();
//            ECDomainParameters ecParams = ecKey.getParameters();
//            (ecParams.getCurve().getFieldSize() + 7) / 8;


            DERSequence result = new DERSequence(
                    new ASN1Encodable[]{
                            new ASN1Integer(BigIntegers.fromUnsignedByteArray(c1x, 0, c1x.length)),
                            new ASN1Integer(BigIntegers.fromUnsignedByteArray(c1y, 0, c1y.length)),
                            new DEROctetString(c3),
                            new DEROctetString(c2)});

            encrypted = result.getEncoded();
        } catch (GeneralSecurityException e) {
            throw (SSLKeyException)new SSLKeyException
                                ("SM2 premaster secret error").initCause(e);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    ECCClientKeyExchange(ProtocolVersion currentVersion,
        ...
        if (!privateKey.getAlgorithm().equals("EC") && !privateKey.getAlgorithm().equals("SM2")) {
            throw new SSLKeyException("Public key not of type SM2");
        }
    ...
    private static SecretKey generatePreMasterSecret(
        ...
        try {
            String s = "TlsGMPremasterSecret";
            KeyGenerator kg = JsseJce.getKeyGenerator(s);
            kg.init(new TlsGMPremasterSecretParameterSpec(
    ...
    void print(PrintStream s) throws IOException {
        s.println("*** ClientKeyExchange, ECC PreMasterSecret, " +
                                                        protocolVersion);
    }
}
